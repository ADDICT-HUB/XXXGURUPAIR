...
app.get('/scan/:code', async (req, res) => {
  ...
  if (info.status === 'open') {
     contentHtml = `<h2>✅ Paired Successfully!</h2>`;
      ...
      // Show QR and the button to request the code (which is automatically generated by Baileys)
      contentHtml = `
        <img src="${qrDataUrl}" width="320" />
        <p style="margin-top: 20px; font-size: 1.1em;">
            OR, use the 6-digit code instead. The code should appear shortly.
        </p>
...
async function startPairingForCode(code) {
  ...
  const sock = makeWASocket({
    ...
    version,
    // THIS IS THE CRUCIAL PART: The callback captures the 6-digit code.
    pairingCode: (pCode) => {
